	...
	static final class FlatMapMain<T, R> extends FlatMapTracker<FlatMapInner<R>>
		...
		public long onSubscribe(CoreSubscription<? extends T> cs, int supportedFusionModes) {
			if (Operators.validate(this.s, cs)) {
				...
				if (requestOrCancel > 0) {
					REQUESTED.lazySet(this, requestOrCancel);
				}

				return Operators.unboundedOrPrefetch(maxConcurrency);
			}
...
//					}
		...
		boolean checkTerminated(boolean d, boolean empty, Subscriber<?> a, R value) {
			if (cancelled) {
				Context ctx = actual.currentContext();
