...
import org.test.reactor_new_api.FluxConcatMap.ConcatMapInner;
import org.test.reactor_new_api.FluxConcatMap.ErrorMode;
import org.test.reactor_new_api.FluxConcatMap.FluxConcatMapSupport;
import org.test.reactor_new_api.FluxConcatMap.WeakScalarSubscription;
			...
			try {
				...
				if (p instanceof Callable) {
					...
					if (inner.isUnbounded()) {
						actual.tryOnNext(result);
						return innerComplete();
					}

					return inner.set(new WeakScalarSubscription<>(result, inner));
				}
...
//						upstream.request(1);
		...
		private boolean maybeOnError(Throwable e, Context ctx, Subscription subscriptionToCancel) {
			if (e == null) {
				return false;
			}
