...
import static org.test.reactor_charcs.Fuseable.NONE;
...
import org.test.reactor_original.Fuseable.QueueSubscription;
...
public abstract class Operators {
	 ...
	 */
	public static final Context enableOnDiscard( Context target, Consumer<?> discardConsumer) {
	 ...
	 */
	public static <T> void onDiscard( T element, Context context) {
   ...
   * @see #onDiscardQueueWithClear(Queue, Context, Function)
   */
	public static void onDiscardMultiple( Collection<?> multiple, Context context) {
   ...
   * @see #onDiscardQueueWithClear(Queue, Context, Function)
   */
	public static void onDiscardMultiple( Iterator<?> multiple, boolean knownToBeFinite, Context context) {
		if (multiple == null) return;
	 ...
	 * @see #onDiscardQueueWithClear(Queue, Context, Function)
	 */
	public static void onDiscardMultiple( Spliterator<?> multiple, boolean knownToBeFinite, Context context) {
		if (multiple == null) return;
		if (!knownToBeFinite) return;
	 ...
	 *
	 */
	public static Throwable onOperatorError( Subscription subscription,
			Throwable error,
			Context context) {
	...
	public static Throwable onOperatorError( Subscription subscription,
			Throwable error,
			 Object dataSignal, Context context) {
	 ...
	 * {@link scheduler.Scheduler}, notably when it was already disposed.
	 * <p>
	...
	public static final BiFunction<? super Throwable, Object, ? extends Throwable> onNextErrorFunction(
      Context context) {
	...
	public static <T> Throwable onNextError( T value, Throwable error, Context context,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> CorePublisher<T> onLastAssembly(
      CorePublisher<T> source) {
	...
	public static RuntimeException onRejectedExecution(Throwable original,
		...
		if (context.hasKey(Hooks.KEY_ON_REJECTED_EXECUTION)) {
			context = context.put(
          Hooks.KEY_ON_OPERATOR_ERROR, context.get(Hooks.KEY_ON_REJECTED_EXECUTION));
		}
	 ...
	 */
	public static <T> Subscription scalarSubscription(
	 ...
	 */
	public static <T> CoreSubscriber<T> serialize(
      CoreSubscriber<? super T> subscriber) {
	...
	static abstract class BaseFluxToMonoOperator<I, O> implements
      InnerOperator<I, O>,
      Fuseable,
			                                                      QueueSubscription<I> {
	...
	abstract static class MultiSubscriptionSubscriber<I, O>
		...
		@Override
		public long onSubscribe(CoreSubscription<? extends I> s, int supportedFusionMode) {
			return set(s);
		}
		...
		public final long set(CoreSubscription<? extends I> s) {
		    if (cancelled) {
	            return -1;
	        }
		...
		final long drainNewSubscription(Subscription a) {
			int missed = 1;
	...
	final static class DrainSubscriber<T> implements CoreSubscriber<T> {
		...
		@Override
		public long onSubscribe(CoreSubscription<? extends T> s, int fusion) {
			return Long.MAX_VALUE;
		}

		@Override
		public long tryOnNext(T o) {
			return 0;
		}
	...
	/*package*/ static class MonoInnerProducerBase<O>
		 ...
		 * @param v the value to discard
		 */
		protected final void discard( O v) {
			Operators.onDiscard(v, actual.currentContext());
		}
