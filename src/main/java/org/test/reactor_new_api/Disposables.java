...
import org.test.reactor_original.Disposable.Swap;
 ...
 * @author David Karnok
 */
static final class SwapDisposable implements Swap {
...
static final class NeverDisposable implements Disposable {
	 ...
	 */
	static <T> boolean set(AtomicReferenceFieldUpdater<T, Disposable> updater, T holder,  Disposable newValue) {
		for (;;) {
	 ...
	 */
	static <T> boolean replace(AtomicReferenceFieldUpdater<T, Disposable> updater, T holder,  Disposable newValue) {
		for (;;) {
			Disposable current = updater.get(holder);
			if (current == DISPOSED) {
				if (newValue != null) {
