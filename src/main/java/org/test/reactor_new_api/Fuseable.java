...
public interface Fuseable {
	 ...
	 */
	interface QueueSubscription<T> extends Queue<T>, CoreSubscription<T> {
		 ...
		 */
		Queue<T> requestFusion(int requestedMode);
	...
	interface SynchronousSubscription<T> extends QueueSubscription<T> {
		...
		@Override
		default Queue<T> requestFusion(int requestedMode) {
