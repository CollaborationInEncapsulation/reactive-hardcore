...
final class FluxOnAssembly<T> extends InternalFluxOperator<T, T> implements Fuseable,
	...
	static void fillStacktraceHeader(StringBuilder sb, Class<?> sourceClass,  String description) {
	...
	static <T> CoreSubscriber<? super T> wrapSubscriber(CoreSubscriber<? super T> actual,
		...
		if(snapshotStack != null) {
			final int characteristics = actual.characteristics();
			if (CoreSubscriber.Characteristics.isConditional(characteristics)) {
	...
	static class AssemblySnapshot {
		 ...
		 */
		AssemblySnapshot( String description, Supplier<String> assemblyInformationSupplier) {
		...
		private AssemblySnapshot(boolean isCheckpoint,  String description,
								  Supplier<String> assemblyInformationSupplier) {
	...
	static final class CheckpointLightSnapshot extends AssemblySnapshot {
		...
		CheckpointLightSnapshot( String description) {
	...
	static final class CheckpointHeavySnapshot extends AssemblySnapshot {
		...
		CheckpointHeavySnapshot( String description, Supplier<String> assemblyInformationSupplier) {
			...
			if (onAssemblyException == null) {
				...
				if (stackTrace.length > 0) {
					...
					for (StackTraceElement stackTraceElement : stackTrace) {
						...
						if (className.startsWith("publisher.") && className.contains("OnAssembly")) {
							continue;
						}
