...
package org.test.reactor_original.scheduler;

import static org.test.reactor_charcs.Exceptions.unwrap;
...
import org.test.reactor_original.Logger;
import org.test.reactor_original.Scannable;
import org.test.reactor_original.Disposable;
import org.test.reactor_original.Exceptions;
import org.test.reactor_original.Loggers;

/**
 * {@link Schedulers} provides various {@link Scheduler} flavors usable by {@link
 * publisher.Flux#publishOn(Scheduler) publishOn} or {@link publisher.Mono#subscribeOn
 * subscribeOn} :
 * <p>
 * <ul>
...
public abstract class Schedulers {
	 ...
	 * is thrown.
	 * <p>
	 * By order of preference, threads backing a new {@link scheduler.Scheduler.Worker} are
	 ...
	 * @param queuedTaskCap maximum number of tasks to enqueue when no more threads can be created. Can be {@link Integer#MAX_VALUE} for unbounded enqueueing.
	 * @param name Thread prefix
	 * @param ttlSeconds Time-to-live for an idle {@link scheduler.Scheduler.Worker}
	 * @return a new {@link Scheduler} that dynamically creates workers with an upper bound to
	 * the number of backing threads and after that on the number of enqueued tasks,
	 * that reuses threads and evict idle ones
	 */
	 ...
	 */
	public static void resetFrom( Snapshot snapshot) {
		if (snapshot == null) {
			resetFactory();
			return;
		}
	 ...
	 * @see #removeExecutorServiceDecorator(String)
	 */
	public static ScheduledExecutorService decorateExecutorService(
      Scheduler owner, ScheduledExecutorService original) {
		synchronized (DECORATORS) {
	...
	/**
	 * Wraps a single {@link scheduler.Scheduler.Worker} from some other
	 * {@link Scheduler} and provides {@link scheduler.Scheduler.Worker}
	...
	public static final class Snapshot implements Disposable {
		...
		private Snapshot( CachedScheduler oldBoundedElasticScheduler,
				 CachedScheduler oldParallelScheduler,
				 CachedScheduler oldSingleScheduler,
				Factory factory) {
	...
	static class CachedScheduler implements
      Scheduler, Supplier<Scheduler>, Scannable {
