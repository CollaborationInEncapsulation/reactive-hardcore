...
import org.test.reactor_original.Disposables;
...
import org.test.reactor_original.Mono;
...
final class BoundedElasticScheduler implements Scheduler,
	...
	private static final SchedulerState<BoundedServices> INIT =
			SchedulerState.init(BoundedServices.SHUTDOWN);
	 ...
	 * (or executors) can be shared by each {@link scheduler.Scheduler.Worker}, so each worker
	...
	public Stream<? extends Scannable> inners() {
		...
		return Stream.concat(Stream.of(services.busyStates.array), services.idleQueue.stream())
		             .filter(obj -> obj != null && obj != BoundedServices.CREATING);
	}
