...
import org.test.reactor_original.Flux;
import org.test.reactor_original.FluxSink;
...
final class SchedulerState<T> {
	...
	private SchedulerState( T initialResource, T currentResource, Mono<Void> onDispose) {
	...
	static <T> SchedulerState<T> transition( T initial, T next, DisposeAwaiter<T> awaiter) {
		return new SchedulerState<T>(
			initial,
			next,
			initial == null ? Mono.empty() :
				Flux.<Void>create(sink -> DisposeAwaiterRunnable.awaitInPool(awaiter, initial, sink, 100))
				    .next());
	}
